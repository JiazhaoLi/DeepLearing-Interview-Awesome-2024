...
- 使用不同类型的损失函数和温度参数来获得更好的知识蒸馏效果。soft label loss, multi-task-distillation loss. contrastive loss.
- 引入额外的信息来提高蒸馏的效果，例如将相似性约束添加到模型训练中。Layer-wise Distillation, aligned in different layers.
- 将蒸馏方法与其他技术结合使用，例如使用多任务学习和迁移学习来进一步改进知识蒸馏的效果。


# 32. Transformer中的Attention计算复杂度以及如何改进？

在标准的Transformer中，attention计算的时间复杂度为O(N^2)，Q K 会生成一个 N*N matrix，其中N是输入序列的长度。为了降低计算复杂度，可以采用以下几种方法：

- 使用自注意力机制，减少计算复杂度。自注意力机制不需要计算输入序列之间的交叉关系，而是计算每个输入向量与自身之间的关系，从而减少计算量。
- 使用Local Attention 局部注意力机制，只计算输入序列中与当前位置相关的子序列的交互 a window of k, O(N^2) -> O(Nk), 从而降低计算复杂度。
  - LongerFormer：
      - 1. Sliding Window Attention (Diagnoal)， k=256,前128后128。
      - 2. Global Attention (whole row and column)： some tokens like CLS.
      - 3. Dilated Sliding window: step=2 sliding window. 
 ![Screenshot 2024-09-07 at 15 52 49](https://github.com/user-attachments/assets/14795919-1dbf-4d6c-bcef-969a9d645703)

- 采用基于近似的方法，例如使用随机化和采样等方法来近似计算，从而降低计算复杂度。
  - Reformer:
    - LSH Locality-sensitive Hashing, nearby value has the same hash value 相似的Q， K gets the same hash value，只计算组内attention and with previous bucket. ![Screenshot 2024-09-07 at 15 52 20](https://github.com/user-attachments/assets/f5d20729-d1ac-4d78-8d57-c4024fc72de8)

- 使用压缩注意力机制，通过将输入向量映射到低维空间来减少计算量，例如使用哈希注意力机制和低秩注意力机制等。
  - Linformer, Linformer 引入了两个线性投影矩阵  E  和  F ，分别作用于  K  和  V  矩阵。
    - 这些投影矩阵将  K  和  V  从  N x d_k  的维度压缩到  k x d_k ，即  k  是一个远小于  N  的固定常数。
	  - 通过这种映射，计算注意力时不再需要生成一个完整的  N x N  矩阵，而是生成一个较小的  N x k  矩阵。
...
P(w) = P(c) * P(w | c) c is the cluster.
...
Flash Attention 是一种高效的注意力机制实现，如共享张量核心和高效的内存使用，以减少内存占用并提高计算速度。这种方法特别适用于具有长序列和大型模型参数的场景，例如自然语言处理和推荐系统。breaking down computations into smaller blocks. 

- 问题背景：传统计算的低效性
- 在深度学习中，很多操作（例如矩阵乘法、Softmax 计算、归一化等）都是逐步执行的，每个操作通常对应一个独立的 GPU kernel。
	- 每个 GPU kernel 执行时需要将数据从 GPU 的全局内存中加载到寄存器或共享内存，完成计算后再写回全局内存。这种反复的数据传输导致了大量的时间花费在内存读写上，而不是实际的计算。
- Fused Kernel Operations 的优化思想
  - 融合计算步骤：将多个相关操作（如矩阵乘法、Softmax 和缩放）融合到一个 GPU kernel 中执行，避免不必要的内存读写。
  - 一次加载多次使用：数据仅需从内存加载一次，可以在一个融合的计算过程中被多次使用，显著减少了内存带宽的消耗和延迟。
  - 减少同步开销：多个操作合并为一个 GPU kernel 后，减少了 GPU 执行过程中的同步和调度开销，从而加速整体运算。
